...
contract PONZIBondingCalculator is IBondingCalculator {
    ...
    function getKValue( address _pair ) public view returns( uint k_ ) {
        uint token0 = IERC20( IPancakeswapV2Pair( _pair ).token0() ).decimals();
        uint token1 = IERC20( IPancakeswapV2Pair( _pair ).token1() ).decimals();
    ...
    function valuation( address _pair, uint amount_ ) external view override returns ( uint _value ) {
        ...
        uint totalSupply = IPancakeswapV2Pair( _pair ).totalSupply();
